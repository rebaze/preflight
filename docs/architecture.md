...
Tag-triggered releases depend on the reusable CI workflow, then build four macOS/Linux archives through GoReleaser. The publication guard records an exact inventory, verifies provenance and source-SBOM attestations plus the checksum signature against the workflow/tag, verifies uploaded draft bytes and publishes only after those checks. A separate, optional Homebrew job verifies the published checksum provenance and uses a tap-scoped GitHub App token. The source SBOM describes the tool's source scan, not a pilot application's dependencies or the runtime container. See [release design](release-design.md) and [operations](releases.md).
