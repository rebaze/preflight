...
Stage 4 keeps optional reasoning in the harness. `skills/preflight/references/investigations.md` defines one-question packets and stop/work limits; its bundled standard-library Python helper validates packet fields, captured identity and citations against a fresh observation, producing only unverified hypotheses or unresolved results. It cannot prove prose entailment or override structured check results. No Go model client or new credential exists. Generic verified observation claims require current captured source citations; missing/stale investigator references cannot be promoted.

Stage 5 adds an independent `capabilities` contract (skill protocol 1 / discovery v1) for plugin/CLI compatibility. Both plugin manifests, public skill files and an installable marketplace are embedded inside each existing CLI archive. Exact archive/checksum/provenance inventory remains four platforms; package tests inspect plugin content and run native compatibility/discovery smoke checks. Homebrew also installs the hidden marketplace directory. Fresh-harness evaluation is separate from those package tests.
