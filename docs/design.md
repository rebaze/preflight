...
Stage 5 packages plugin candidate 0.1.0-rc.4 with CLI skill protocol 1 and explicit capability checking. Installation selects an exact archive or reviewed source commit, never arbitrary latest binaries/hooks. Four existing CLI archives carry the plugin inside their signed/attested contents; no fifth release asset is added. Fresh-harness timing and onboarding are measured using synthetic sources without telemetry, distinguishing agent evaluations from human observations and recording misses.

Stage 2 review clarification: result repository, revision and producer identities all participate in matching. Both head and merge checks belong to the target repository even for fork PRs. A missing merge SHA is unknown, distinct from a known merge SHA with a complete empty result set (GitHub head fallback). Legacy status creator names cannot establish App identity, including in saved observations.
