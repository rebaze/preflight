...
`inspect --github [--pr NUMBER]` uses existing authenticated `gh api` access with GET requests only, fixed github.com host, bounded output/pages/requests/time, and no stored Preflight credentials. It recognizes a direct HTTPS/SSH github.com origin; SSH aliases and other providers are explicitly unsupported. PR lookup is scoped to that origin repository; the adapter does not guess an upstream PR repository from a fork origin. Select `--pr` when necessary; a unique open PR for the local branch is used otherwise. The selected PR base wins; otherwise explicit `--base` names the target branch or the repository default branch is displayed. A commit-only base cannot establish a GitHub branch gate.
...
## Saved observations and comparisons

`inspect --output /private/outside-checkouts/before.json` creates a new owner-readable file; its parent must already exist. Existing files are never overwritten. Symlink destinations, source/Git directories and other Git checkouts are refused. No trusted-policy state is initialized or migrated. `--compare before.json` strictly decodes a prior observation and reports changes; combine it with a fresh `--output after.json` to retain both. Observations have a 16 MiB serialized limit and remain user-controlled, unauthenticated local evidence.

Comparison requires compatible repository, worktree, branch and comparison-target identities. Moving comparison commits is described separately. It identifies changed captured inputs, configured requirements, new/resolved findings and stale/unknown applicability. Incomplete collection cannot establish removal or resolution. A result transition is comparable only with matching requirement, source repository, producer and head/merge subject; successful neutral/skipped checks are not labeled observed passing tests. The two observations can bound last-observed-passing/first-observed-failing times; they never identify the originating commit or cause by themselves.

Structural CI explanations recognize deleted workflows, removal of a literal `pull_request` trigger and deletion of a unique literal job associated with an observed required GitHub Actions check/App. Each includes file/line and before/after values. Dynamic names, matrices, reusable jobs, ambiguous mappings, aliases and unsupported YAML stay unverified. This is a constrained structural observation, not a shell/YAML interpreter, Conftest verdict or test rerun. Restoration is established from current source structure, not merely from a previously reported change disappearing from a diff. A failed check whose cause is unsupported remains unknown.

Fork PRs retain the fork as source-code identity, but required-check evidence is queried and matched in the target repository where PR workflows and gates belong. A same-SHA/name/App result from another repository cannot satisfy that target's requirement. Effective-rule identifiers are stable across response ordering. Duplicate or incomplete rule details retain explicit ambiguity; their omission cannot establish removal in a comparison.
