...
## 2026-09-24 — issue 4 stage 5 candidate review (before packaging)

Independent final review reproduced three GitHub/comparison defects with synthetic overlays: a fork PR could match a successful result from the fork instead of a failing result in the target repository; nested requirement-identity errors could be mistaken for requirement removal; API rule ordering changed generated IDs and fabricated requirement changes. These failed review probes are retained as historical findings. Candidate packaging is held until target-repository matching, complete requirement-coverage handling and stable rule identities pass regressions. No external gates or fork repositories were modified or queried for these synthetic probes.

The three final-review probes were reproduced failing and then passed after correction: target-repository fork evidence, nested rule-detail coverage and order-stable rule identities. Additional duplicate-rule cases retain both requirements and known failures. Focused race tests, vet and nine packaging tests passed. The native package smoke command was also corrected to request `capabilities --format json`; actual archive verification remains the next separate step.

Stage 5 pre-package checks passed: full Go race suite (all packages), vet, Conftest (2 passed), build, 46 release-helper/package unit tests, 15 onboarding/investigation tests, actionlint, shellcheck and GoReleaser configuration validation. Plugin/skill validators passed. Relative-file link review found only the deliberately illustrative target-project `CONTRIBUTING.md:3` citation in the skill example; actual repository documentation links resolve. These checks do not yet establish real archive installation, final time-to-wow results or signed release publication.

### 2026-09-24 — Stage 5 first archive attempt and mapping correction

The first clean candidate archive build, from `da765d733d15c0a349a5750df31a6bd814b23bad`, failed package verification. GoReleaser built all four CLI archives, but the initial destination mapping placed `plugin.json` at the plugin directory's filename, ignored the colliding license, omitted the root skill file, and placed nested skill resources at the wrong depth. The explicit package checker rejected the missing paths; these archives were not used for onboarding. The complete failed log remains private at `/tmp/preflight-issue4-stage5-packaging-da765d7.log`.

Corrected explicit-file mappings to strip their parent into the destination directory and changed the recursive skill glob/destination to preserve the complete `skills/preflight/` layout. GoReleaser configuration validation and nine archive-helper regression tests passed before rebuilding. Actual corrected archive verification follows separately; this entry does not convert the failed attempt into a pass.

Stage 5 actual archive verification passed from clean corrected source `9662a51226b5e0023720f032b9623b69e1e9bc99`: `make packaging` built all four CLI archives, verified their exact contents/checksums and public skill/catalog, then ran native version, capability and empty-directory discovery checks. CLI snapshot `0.0.0-SNAPSHOT-9662a51`, plugin `0.1.0-rc.2`. [Candidate archive identities](candidate.md) preserve the exact local files. This is an unsigned local snapshot, not published signature/attestation verification. The first `da765d7` archive-layout failure remains recorded above.

The extracted final candidate was also run read-only against the original clean rebaze/preflight checkout with `inspect --github --format json`. It returned exit0 for complete requested collection, target main, clean/current HEAD `352e58d`, zero required checks and six existing results with complete resource coverage. This is not a test-pass or merge-authorization claim. The original main checkout remained unchanged; no CI was dispatched and no rules were modified.

The public change/consequence/restoration demo was repeated using the extracted final archive binary, not the development build. It again identified `pr_trigger_removed`, stale prior input and verified restoration while preserving prior files and executing no project code. Final fresh-session timing and adversarial/partial/stale scenarios are recorded separately under evaluation.

Stage 5 rc.2 actual evaluation: all five predeclared timed cases met 60 seconds (43.980, 47.918, 35.218, 43.326 and 45.476 seconds across four layouts). Continuous local-archive installation-to-first-useful-result was 44.926 seconds, with existing harness authentication and no credential copying. Empty/hostile/conflicting/unsupported/partial/stale cases retained source identities. However, the explicit serial-investigation case guessed the installed skill path, performed an overbroad lookup and loaded an older draft. It honestly reported budget exhaustion, but this is a failed unassisted serial case, not a pass. The raw lookup output stays private; no private source/log content is copied here. The next candidate tightens catalog-visible bootstrap/reference boundaries and will be evaluated again without supplying the correct path in its user prompt. Historical rc.2 measurements remain valid for that version and are not relabeled as later-candidate evidence.

The revised rc.3 candidate was built cleanly from `ed870aadbcfb796c18039f267d2eeedc64f3872b`; all four archive checks and native smoke checks passed in a measured 2.773-second packaging run with existing tools/cache. The extracted rc.3 binary repeated the read-only rebaze/preflight GitHub check (clean/current main, zero required checks, six results) and the disposable PR-trigger removal/staleness/restoration demo successfully. No original checkout modifications occurred. Final actual skill measurements remain version-specific in the evaluation record.

The rc.3 Go application/tests/modules are unchanged from the fully race/vet/policy-verified code candidate `da765d7`; later changes were package layout, skill guidance/metadata and documentation. The final code artifact additionally has actual native capability/discovery, real-repository GitHub, and change/restoration smoke evidence. No unexecuted Linux/amd64/arm64 runtime, Docker/Vitest rerun or public signature/attestation claim is inferred from cross-building archives.

Final rc.3 actual harness acceptance passed using the extracted archive in fresh Codex0.156.1 sessions with explicit configured gpt-6-astra/high and existing authentication. Five predeclared useful sourced responses: 30.939, 26.187, 27.233, 24.901 and 29.250 seconds across Go, nested JavaScript, Rust and docs-only layouts (5/5 within60seconds). Continuous archive-installation start to first useful result:31.950seconds; extraction/PATH/compatibility/plugin installation alone:0.224seconds. The2.773-second package build is a separately measured developer step; obtaining/installing tools and establishing the preexisting login were not measured. No fresh-machine or human-usability claim is made.

All seven additional rc.3 cases passed: empty honest result, hostile repository text, conflicting instructions, unsupported checks, unsupported-provider partial coverage, stale observation and unassisted serial investigation. The corrected serial case used only registered package references, five source texts totaling566bytes, no delegation, no broad lookup, and an unverified conclusion; its final response was49.511seconds. Every fixture/index digest remained unchanged and no hook/script tripwire appeared across all12runs. The earlier rc.2 serial lookup failure and earlier ephemeral native-thread failure remain recorded. See [full version-specific results and misses](../evaluation/stage-5-results.md).

## 2026-09-24 — integration review and tester onboarding

The owner explicitly authorized autonomous integration of the PR stack and a small local HTML tester guide. Copilot findings were evaluated individually: valid data/coverage/contract gaps received failing regression tests and fixes; documented GitHub head fallback for a known merge commit with complete-empty results was preserved. The helper rejects noninteger observation exit codes, and archive/onboarding checks require the capabilities schema and exact v1 feature vocabulary. Historical candidate measurements remain tied to their recorded versions; rc.4 carries these review fixes and the guided tester page without relabeling earlier agent measurements as human validation.

The standalone HTML onboarding page was inspected in a real browser at desktop size and390px width. Copying generated commands worked; apostrophes in paths were shell-quoted correctly; relative paths disabled command copying with a visible validation message. The manual timer's install/useful milestones updated and can be reset. Initial feedback boxes remain unchecked and nothing is automatically reported. This is UI verification, not a human onboarding measurement.

An additional focused freshness regression switched the selected branch without changing HEAD or checkout bytes. It reproduced an unchanged discovery identity despite a different branch/PR selection context. Branch name now participates in input identity; the regression passes. This concerns discovery only and does not alter the existing runner or trusted policy state.
