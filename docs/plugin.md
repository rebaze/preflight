...
Save with `--output /private/preflight/before.json` in an existing private parent outside all Git checkouts, then re-inspect with `--compare /private/preflight/before.json`. Each output file must be new; old observations are preserved. Comparisons explain changed inputs/requirements and stale evidence. Structural CI explanations are limited to supported literal forms; ambiguous causes remain unknown. Bounded investigations follow in the next stage. Existing deterministic checks remain limited to the explicitly configured supported frontend profile and approved Docker execution. A remote matching result is never a locally executed check or release authorization.
