...
Save with `--output /private/preflight/before.json` in an existing private parent outside all Git checkouts, then re-inspect with `--compare /private/preflight/before.json`. Each output file must be new; old observations are preserved. Comparisons explain changed inputs/requirements and stale evidence. Structural CI explanations are limited to supported literal forms; ambiguous causes remain unknown. Existing deterministic checks remain limited to the explicitly configured supported frontend profile and approved Docker execution. A remote matching result is never a locally executed check or release authorization.

## Optional investigations

Ask a narrow question after the initial briefing, such as whether a declared test command establishes a documented compatibility expectation. Delegation is off by default. When requested and supported, the skill may use at most two native investigators, each limited to one question, 60 seconds, six sources and 64 KiB. The first useful briefing precedes this work. Read-only harness permissions are preferred; the skill cannot grant or enforce a sandbox by itself. Missing or failed native delegation falls back to the same bounded serial investigation. It never delegates repository commands to bypass execution restrictions.

The [investigation protocol](../skills/preflight/references/investigations.md) provides a strict packet schema and a Python standard-library helper that validates source references, digests and fresh observation identity. It emits only unverified hypotheses or unresolved results and leaves original evidence unchanged. It cannot prove that natural-language reasoning follows from a source; the parent must reject unsupported execution claims and preserve disagreement. Python is optional for this helper, not a discovery prerequisite. See [actual investigation evaluations](../evaluation/stage-4-results.md), including a native-delegation failure and its serial fallback.
