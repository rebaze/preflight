#!/usr/bin/env python3
"""Stage Preflight release assets and verify their exact bytes before publication.

Python 3.9+, gh and cosign. Network access belongs here, never in the Preflight CLI.
...
def validate_inventory(data, args):
    ...
    expected_archives = {f"preflight_{args.tag[1:]}_{os}_{arch}.tar.gz"
                         for os in ("darwin", "linux") for arch in ("amd64", "arm64")}
    if {name for name in files if archive(name)} != expected_archives:
        raise ValueError("release inventory must contain exactly the four expected archives")
...
def publish(args):
    ...
    with tempfile.TemporaryDirectory(prefix="preflight-release-") as temporary:
        snapshot = Path(temporary) / "assets"
        snapshot.mkdir()
        for name in files:
            shutil.copyfile(args.stage / name, snapshot / name)
        check_files(snapshot, files)
        bundle_name = "preflight-" + args.tag + ".intoto.jsonl"
            ...
            if archive(name) or name == "checksums.txt":
                ...
                for predicate in predicates:
                    ...
                    if predicate == SBOM:
                        ...
                        source = json.loads((snapshot / ("preflight-" + args.tag + "-source.cdx.json")).read_text())
