...
class ReleaseTest(unittest.TestCase):
    ...
    def test_stage_refuses_missing_platform_even_with_consistent_checksums(self):
        (self.dist / self.archive).unlink()
        checksums = self.dist / "checksums.txt"
        checksums.write_text("\n".join(line for line in checksums.read_text().splitlines()
                                     if not line.endswith(self.archive)) + "\n")
        self.assert_blocked(self.command("stage"), "archives")

    def test_prerelease_remains_prerelease(self):
        old = self.sbom
        self.tag = "v1.2.3-rc1"
        self.sbom = "preflight-" + self.tag + "-source.cdx.json"
        (self.dist / old).rename(self.dist / self.sbom)
        for path in self.dist.glob("*.tar.gz"):
            path.rename(path.with_name(path.name.replace("1.2.3", "1.2.3-rc1")))
        checksums = self.dist / "checksums.txt"
        checksums.write_text(checksums.read_text().replace("1.2.3", "1.2.3-rc1"))
        data = json.loads(self.bundle.read_text())
        data["subjects"] = {name.replace("1.2.3", "1.2.3-rc1"): sha
                            for name, sha in data["subjects"].items()}
        data["subjects"]["checksums.txt"] = hashlib.sha256(checksums.read_bytes()).hexdigest()
